...
## Rules
1. **Descriptive only:** do not judge, do not recommend, do not assign severity.
2. **Focus on the hierarchy:** page → regions → containers → components → elements.
3. If information is uncertain from the screenshot, say so plainly (e.g., “Text is not legible at this resolution”).
4. Do not reference previous pages unless explicitly asked.
