...
export default function App({ state, actions, helpers }: any) {
 ...
 // Local state
  ...
  return (
    <div className="w-full max-w-md md:max-w-xl xl:max-w-md mx-auto min-h-screen pb-32 relative overflow-x-hidden font-sans text-white">
        ...
        {/* --- ITEM DETAIL MODAL --- */}
        {selectedProduct && (
            <div className="fixed inset-0 z-[100] flex items-end md:items-center xl:items-end justify-center bg-black/80 backdrop-blur-sm animate-slide-up">
                <div className="w-full max-w-md md:max-w-xl xl:max-w-md bg-[#18181B] border-t-8 border-x-8 border-[#F97316] h-auto max-h-[95vh] md:max-h-[85vh] xl:max-h-[95vh] flex flex-col overflow-hidden shadow-lg rounded-t-[3rem] md:rounded-2xl xl:rounded-none xl:rounded-t-[3rem] relative">
                    <div className="relative shrink-0">
                        ...
                        <div className="relative w-full h-72 md:h-52 xl:h-72 shrink-0 overflow-hidden border-b-8 border-[#F97316] rounded-b-[2rem] bg-[#000]">
                    ...
                    <div className="px-8 pt-8 pb-6 relative overflow-y-auto no-scrollbar flex-1 bg-[#27272A]">
                            ...
                            {selectedProduct.options && selectedProduct.options.length > 0 && (
                                <div className="space-y-4">
                                    {selectedProduct.options.map((opt: any, index: number) => (
                                        <div key={index} className="bg-[#18181B] border-4 border-[#3F3F46] rounded-2xl p-4 shadow-sm">
                                            <div className="flex justify-between items-center mb-3">
                                                <label className="text-lg font-bold text-white garfield-font tracking-wide">
                                                    {opt.name}
                                                    {opt.required && <span className="text-[#EF4444] ml-2 text-sm">* REQUIRED</span>}
                                                </label>
                                                <span className="text-sm font-bold text-[#A1A1AA] bg-[#27272A] px-2 py-1 rounded-md border border-[#3F3F46]">
                                                    {opt.type === 'single' ? 'CHOOSE ONE' : 'CHOOSE MANY'}
                                                </span>
                                            </div>
                                                ...
                                                {opt.choices.map((choice: any, cIndex: number) => {
                                                    ...
                                                    return (
                                                        <div
                                                            ...
                                                            className={`flex items-center justify-between p-3 rounded-xl border-4 cursor-pointer transition-all ${
                                                                isSelected
                                                                    ? 'bg-[#FBBF24] border-[#F97316] text-black shadow-sm transform -translate-y-0.5'
                                                                    : 'bg-[#27272A] border-[#3F3F46] text-white hover:border-[#F97316]'
                                                            }`}
                                                        >
                                                            <div className="flex items-center gap-3">
                                                                <div className={`w-6 h-6 rounded-md border-2 flex items-center justify-center ${
                                                                    isSelected ? 'bg-black border-black text-[#FBBF24]' : 'bg-[#18181B] border-[#52525B]'
                                                                }`}>
                                                                    {isSelected && <Icon name="check" size={16} strokeWidth={4} />}
                                                                </div>
                                                                {(choice.image_url || choice.image_name) && (
                                                                    <img src={choice.image_url || getMenuUrl(choice.image_name)} alt={choice.name} className="w-10 h-10 object-cover rounded-md border-2 border-black/20" />
                                                                )}
                                                                <span className="font-bold garfield-font text-lg">{choice.name}</span>
                                                            </div>
                                                            {Number(choice.price) > 0 && (
                                                                <span className={`font-black text-xl garfield-font ${isSelected ? 'text-black' : 'text-[#F97316]'}`}>
                                                                    +{Number(choice.price)}.-
                                                                </span>
                                                            )}
                                                        </div>
                    ...
                    <div className="shrink-0 p-5 md:p-6 bg-[#27272A] grid grid-cols-2 gap-4 border-t-8 border-[#F97316] relative z-30">
        ...
        {activeTab === 'cart' && (
             <div className="fixed inset-0 z-[100] flex items-end md:items-center xl:items-end justify-center bg-black/80 backdrop-blur-sm animate-slide-up">
                 <div className="w-full max-w-md md:max-w-xl xl:max-w-md mx-auto bg-[#18181B] border-t-8 border-[#F97316] flex flex-col shadow-lg h-[85vh] rounded-t-[3rem] md:rounded-2xl xl:rounded-none xl:rounded-t-[3rem] relative">
