...
export default function App({ state, actions, helpers }: any) {
  ...
  const currentPriceObj = selectedProduct
    ? {
        ...
        original: basePriceObj.original ? (basePriceObj.original + toppingTotal) : (basePriceObj.final + basePriceObj.discount + toppingTotal)
      }
  ...
  return (
    ...
    <div className="w-full max-w-md md:max-w-xl xl:max-w-md mx-auto min-h-screen pb-32 relative overflow-x-hidden font-sans text-[#2e1065] bg-[#faf5ff] border-x border-purple-200">
        ...
        {/* --- ITEM DETAIL MODAL --- */}
        {selectedProduct && (
            <div className="fixed inset-0 z-[110] flex items-end md:items-center xl:items-end justify-center bg-indigo-950/80 backdrop-blur-sm animate-fade-in p-0">
                <div className="w-full max-w-md md:max-w-xl xl:max-w-md bg-white h-auto max-h-[95vh] md:max-h-[85vh] xl:max-h-[95vh] flex flex-col overflow-hidden shadow-2xl border-t-8 border-emerald-500 rounded-t-[3rem] md:rounded-2xl xl:rounded-none xl:rounded-t-[3rem] relative">
                            ...
                            {selectedProduct.options && Array.isArray(selectedProduct.options) && selectedProduct.options.length > 0 && (
                                <div className="space-y-6">
                                    {selectedProduct.options.map((opt: any, groupIdx: number) => {
                                        ...
                                        return (
                                            <div key={groupIdx} className="border-t border-purple-100 pt-4 first:border-t-0 first:pt-0">
                                                <label className="block text-sm font-bold text-indigo-900 mb-3 ml-1 tracking-wide uppercase flex justify-between items-center">
                                                    <span>
                                                        {opt.name}
                                                        {opt.required && <span className="text-pink-600 ml-1 font-sans font-normal text-xs">(จำเป็น)</span>}
                                                    </span>
                                                    <span className="text-[10px] text-purple-400 font-bold normal-case">
                                                        {opt.type === 'single' ? 'เลือกได้ 1 รายการ' : 'เลือกหลายรายการ'}
                                                    </span>
                                                </label>
                                                    ...
                                                    {opt.choices.map((choice: any) => {
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                className={`p-3 border-2 transition-all flex flex-col items-center justify-center gap-2 rounded-xl active:scale-95 text-center h-full min-h-[5.5rem]
                                                                    ${isSelected
                                                                        ? 'bg-purple-50 border-purple-500 text-purple-700 shadow-md font-bold'
                                                                        : 'bg-white border-purple-100 text-slate-500 hover:border-purple-300'}`}
                                                            >
                                                                {imgUrl && (
                                                                    <div className="w-12 h-12 rounded-lg overflow-hidden shrink-0 bg-purple-50 border border-purple-100 flex items-center justify-center">
                                                                        <img src={imgUrl} alt={choice.name} className="w-full h-full object-cover" />
                                                                    </div>
                                                                )}
                                                                <div className="flex flex-col items-center leading-tight">
                                                                    <span className="text-xs font-bold leading-tight line-clamp-2">{choice.name}</span>
                                                                    {Number(choice.price || 0) > 0 && (
                                                                        <span className="text-xs font-black text-emerald-600 mt-1">+{choice.price}.-</span>
                                                                    )}
                                                                </div>
                                                            </button>
                                                        );
                    ...
                    <div className="shrink-0 w-full p-5 md:p-6 bg-white border-t border-purple-100 grid grid-cols-2 gap-4 z-30">
        ...
        {activeTab === 'cart' && (
            <div className="fixed inset-0 z-[130] flex items-end justify-center bg-indigo-950/90 backdrop-blur-sm animate-fade-in md:items-center">
                <div className="w-full max-w-md md:max-w-xl xl:max-w-md mx-auto bg-white border-t-[10px] border-emerald-500 flex flex-col shadow-2xl h-[92vh] md:h-auto md:max-h-[90vh] relative rounded-t-[4rem] md:rounded-2xl xl:rounded-none xl:rounded-t-[4rem]">
