...
export default function App({ state, actions, helpers }: any) {
  ...
  return (
    ...
    <div className="w-full max-w-md md:max-w-xl xl:max-w-md mx-auto min-h-screen pb-32 relative overflow-x-hidden font-sans text-[#1F2937]">
        ...
        {selectedProduct && (
            <div className="fixed inset-0 z-[100] flex items-end md:items-center xl:items-end justify-center bg-black/80 backdrop-blur-sm animate-fade-in">
                <div className="w-full max-w-md md:max-w-xl xl:max-w-md bg-white border-t-8 border-x-8 border-black h-auto max-h-[95vh] md:max-h-[85vh] xl:max-h-[95vh] flex flex-col overflow-hidden shadow-2xl rounded-t-[4rem] md:rounded-2xl xl:rounded-none xl:rounded-t-[4rem] relative animate-image-pop">
                        ...
                        <div className="space-y-6">
                            ...
                            {selectedProduct.options && selectedProduct.options.length > 0 && (
                                <div className="mt-6 space-y-6">
                                    {selectedProduct.options.map((opt: any, groupIndex: number) => (
                                        <div key={groupIndex} className="bg-white p-5 rounded-2xl border-4 border-black shadow-[4px_4px_0_#000]">
                                            <div className="flex justify-between items-center mb-4">
                                                <h3 className="text-xl font-bold text-black wacky-font tracking-wide uppercase">
                                                    {opt.name}
                                                </h3>
                                                {opt.required && (
                                                    <span className="bg-[#DC2626] text-white text-[12px] font-black px-2 py-1 rounded border-2 border-black wacky-font tracking-widest">
                                                        REQUIRED
                                                    </span>
                                                )}
                                            </div>
                                                ...
                                                {opt.choices.map((choice: any, choiceIdx: number) => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`flex flex-col items-center justify-between p-3 rounded-xl border-4 transition-all w-full min-h-[90px]
                                                                ${isSelected
                                                                    ? 'bg-[#7C3AED] border-black shadow-[4px_4px_0_#000] -translate-y-1 text-white'
                                                                    : 'bg-white border-gray-300 text-gray-400 hover:border-black hover:text-black'}`}
                                                        >
                                                            {(choice.image_url || choice.image_name) && (
                                                                <div className="w-12 h-12 mb-2 bg-[#FEF3C7] rounded-full border-2 border-black overflow-hidden shrink-0">
                                                                    <img src={choice.image_url || getMenuUrl(choice.image_name)} alt={choice.name} className="w-full h-full object-cover" />
                                                                </div>
                                                            )}
                                                            <span className={`text-sm font-black tracking-wide text-center leading-tight ${isSelected ? 'text-white' : 'text-black'}`}>
                                                                {choice.name}
                                                            </span>
                                                            {Number(choice.price) > 0 && (
                                                                <span className={`text-sm font-black mt-1 ${isSelected ? 'text-yellow-300' : 'text-[#DC2626]'}`}>
                                                                    +{Number(choice.price)}.-
                                                                </span>
                                                            )}
                                                        </button>
                            ...
                            <div className="flex items-center justify-between py-4 mt-2">
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-5xl font-black text-[#DC2626] wacky-font">{finalPriceWithOpts * qty}.-</p>
                                </div>
                            </div>
                    ...
                    <div className="shrink-0 w-full p-5 md:p-6 bg-white border-t-8 border-black grid grid-cols-2 gap-5 z-30">
        ...
        <div id="orderSummary" className={`fixed bottom-0 left-0 right-0 max-w-md md:max-w-xl xl:max-w-md mx-auto bg-white border-t-[8px] border-black z-[140] flex flex-col shadow-2xl h-[92vh] rounded-t-[3rem] md:rounded-2xl xl:rounded-none xl:rounded-t-[3rem] transition-transform duration-300 ${activeTab === 'cart' ? 'translate-y-0' : 'translate-y-full'}`}>
