...
export default function App({ state, actions, helpers }: any) {
  ...
  return (
    ...
    <div className="w-full max-w-md md:max-w-xl xl:max-w-md mx-auto min-h-screen pb-32 relative overflow-x-hidden font-sans text-white">
        ...
        {selectedProduct && (
            <div className="fixed inset-0 z-[100] flex items-end md:items-center xl:items-end justify-center bg-[#3E2723]/80 backdrop-blur-sm animate-fadeIn">
                <div className="w-full max-w-md md:max-w-xl xl:max-w-md bg-[#FFF8E1] border-t-8 border-x-8 border-[#5D4037] h-auto max-h-[95vh] md:max-h-[85vh] xl:max-h-[95vh] flex flex-col overflow-hidden shadow-[0_-10px_0_#3E2723] rounded-t-[3rem] md:rounded-[3rem] xl:rounded-none xl:rounded-t-[3rem] relative">
                                ...
                                <div className="px-6 py-2 bg-[#D32F2F] text-white font-normal text-3xl survival-font rounded-xl border-4 border-[#FFF] shadow-[4px_4px_0_#3E2723] transform -rotate-3 flex flex-col items-center leading-none">
                                    {currentPriceObj.discount > 0 && (
                                        <span className="text-sm line-through text-[#FFCDD2] decoration-white decoration-2 mb-1">
                                            {(currentPriceObj.original || currentPriceObj.final + currentPriceObj.discount) + toppingTotal}
                                        </span>
                                    )}
                    ...
                    <div className="px-8 pt-8 pb-6 flex-1 overflow-y-auto no-scrollbar relative bg-[#FFF3E0]">
                        ...
                        <div className="space-y-5 relative z-10">
                            ...
                            <div className="flex items-center justify-between py-4 mt-2 bg-[#FFF8E1] border-4 border-[#8D6E63] rounded-2xl p-4 shadow-[4px_4px_0_#5D4037]">
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-5xl font-normal text-[#3E2723] survival-font">{finalPriceWithOpts * qty}.-</p>
                                </div>
                            </div>

                            {/* --- PRODUCT OPTIONS --- */}
                            {selectedProduct.options && Array.isArray(selectedProduct.options) && selectedProduct.options.map((opt: any, optIndex: number) => (
                                <div key={opt.id || optIndex} className="space-y-2">
                                    <label className="block text-lg font-bold text-[#5D4037] mb-2 survival-font ml-1 tracking-wide uppercase">
                                        {opt.name} {opt.required && <span className="text-[#D32F2F]">*</span>}
                                    </label>
                                    <div className="grid grid-cols-2 gap-3">
                                        {opt.choices && Array.isArray(opt.choices) && opt.choices.map((choice: any) => {
                                            const isSelected = (selectedOptions[optIndex] || []).some(
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`p-2 rounded-xl border-4 transition-all flex flex-col justify-between survival-font min-h-[4rem] text-left
                                                        ${isSelected
                                                            ? 'bg-[#FFCC80] border-[#E65100] shadow-[3px_3px_0_#3E2723] text-[#3E2723]'
                                                            : 'bg-[#FFF8E1] border-[#D7CCC8] text-[#A1887F] hover:border-[#8D6E63] hover:text-[#5D4037]'}`}
                                                >
                                                    {choiceImgUrl && (
                                                        <div className="w-full h-24 rounded-lg overflow-hidden mb-2 bg-[#EFEBE9] border-2 border-[#D7CCC8] shrink-0">
                                                            <img src={choiceImgUrl} className="w-full h-full object-cover" />
                                                        </div>
                                                    )}
                                                    <div className="flex-1 flex flex-col justify-center px-1">
                                                        <span className="text-sm font-bold leading-tight">{choice.name}</span>
                                                        {Number(choice.price || 0) > 0 && (
                                                            <span className="text-xs font-semibold mt-1">+{Number(choice.price)}.-</span>
                                                        )}
                                                    </div>
                                                </button>
                                            );
        ...
        <div id="orderSummary" className={`fixed bottom-0 left-0 right-0 max-w-md md:max-w-xl xl:max-w-md mx-auto bg-[#FFF8E1] border-t-8 border-[#5D4037] flex flex-col shadow-[0_-10px_0_#3E2723] h-[85vh] rounded-t-[3rem] md:rounded-[3rem] xl:rounded-none xl:rounded-t-[3rem] transition-transform duration-300 ${activeTab === 'cart' ? 'translate-y-0' : 'translate-y-full'}`}>
